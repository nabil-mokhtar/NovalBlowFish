...
class StatisticalTestApp:
    def __init__(self, root):
        ...
        self.compare_button = tk.Button(top_frame, text="Compare Blowfish and Noval Blowfish",
                                        command=lambda: self.select_algorithm(False))
        self.compare_button.pack(side=tk.LEFT, padx=10)

        self.compare_button_2 = tk.Button(top_frame, text="Compare Blowfish and Noval Blowfish Image",
                                          command=lambda: self.select_algorithm(True))
        self.compare_button_2.pack(side=tk.LEFT, padx=10)
    ...
    def select_algorithm(self, image_flag):
        if image_flag:
            ...
            try:
                # Run tests for both algorithms and display in respective tabs
                self.run_tests(self.readfromfile("binary_data_for_text.txt"), self.result_tree_triple)
                self.run_tests(self.readfromfile("binary_data_for_text_single.txt"), self.result_tree_single)

                # self.run_tests(self.generate_random_test(rand_string), self.result_tree_single)
                #
                # self.run_tests(self.generate_random_test_t(rand_string), self.result_tree_triple)
            except Exception as e:
                messagebox.showerror("Error", f"An error occurred: {str(e)}")
