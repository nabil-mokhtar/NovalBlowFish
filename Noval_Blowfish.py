...
class App:
    ...
    def encrypted_blowfish_text_single(self, key1, rand_string):
    ...
    def encrypted_blowfish_image_single(self, key1, path):
        blowfish = GetBlowfish(key1)
        encrypted = blowfish.encrypt_image(path, "encrypted_fish_single.jpg")
        binary_data = ''.join(format(byte, '08b') for byte in encrypted)
        return binary_data
    ...
    def encrypt_image(self):
        ...
        self.single_blowfish_bytes = self.encrypted_blowfish_image_single(key1, self.image_path)
        # Encrypt the image
        encrypted_bytes = self.triple_blowfish.encrypt_image(self.image_path, self.encrypted_image_path)
        # Create noise image
        self.noise_image = self.generate_noise_image()

        image = Image.open(self.image_path)
        binary_data = image.tobytes()
        self.binary_data = ''.join(format(byte, '08b') for byte in encrypted_bytes)
        ...
        with open("binary_data_for_image_single.txt", "w") as f:
            f.write(self.single_blowfish_bytes[:1000000])
    ...
    def show_images(self, original, noise_path, decrypted):
        ...
        original = original.resize(max_size, Image.LANCZOS)
        noise_image = noise_image.resize(max_size, Image.LANCZOS)
        decrypted = decrypted.resize(max_size, Image.LANCZOS)
        ...
        pop_up.geometry(
            f"{pop_up_width}x{pop_up_height}+{(screen_width // 2) - (pop_up_width // 2)}+{(screen_height // 2) - (pop_up_height // 2)}")
    ...
    def encrypt_text(self):
        ...
        try:
            ...
            print(binary_data)
            self.encrypted_blowfish_text_single_bytes = self.encrypted_blowfish_text_single(key1, plain_text)

            with open("binary_data_for_text.txt", "w") as f:
                f.write(binary_data)

            with open("binary_data_for_text_single.txt", "w") as f:
                f.write(self.encrypted_blowfish_text_single_bytes)
